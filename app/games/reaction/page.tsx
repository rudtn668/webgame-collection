...
export default function Page() {
  ...
  try {
    ...
    const r = await fetch(`/api/submit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ game: 'reaction', name: name.trim(), score: result, runId }) });
  ...
  function click() {
    if (phase === 'waiting') {
      ...
      setPhase('idle');
      setError('성급했어요! 초록색으로 바뀐 후 클릭하세요.');
      return;
    }
    if (phase === 'ready') {
  ...
  return (
    <main className="container">
      ...
      <div className="hud">
        ...
        <div>결과: <strong>{result ? msFmt(result) : '-'}</strong></div>
      </div>
      ...
      <style jsx>{`
        ...
        .arena { display:flex; align-items:center; justify-content:center; height: 320px; border-radius: 12px; user-select:none; }
        .arena.idle { background:#222; }
        .arena.waiting { background:#7a1f1f; }
        .arena.ready { background:#1f7a2f; }
        .arena.done { background:#2b2b2b; }
        .primary { padding: 10px 16px; border-radius: 10px; border:0; background:#444; color:#fff; font-weight:700; }
        .submitBox { display:flex; flex-direction:column; gap:10px; align-items:center; }
